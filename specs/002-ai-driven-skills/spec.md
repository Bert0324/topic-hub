...
### User Story 1 - Skill Uses AI in Lifecycle Hooks (Priority: P1)

A Skill developer creates a Type Skill (e.g., "alert") that uses AI in its `onTopicCreated` hook. When a new alert topic is created, the Skill calls the AI service to analyze the alert metadata and auto-generate a severity assessment and suggested first-responder actions. The AI response is included in the topic card posted to the IM group. The AI call is made through a standard `AiService` that the Skill receives via dependency injection — the Skill doesn't need to know which AI provider is configured.

**Why this priority**: This is the core capability — Skills that can leverage AI become significantly more intelligent. Without the AI service infrastructure, no AI-powered Skill feature is possible. This is the foundation all other AI use cases build on.

**Independent Test**: Can be tested using a test-only Skill fixture (not bundled in `skills/`) that calls `AiService.complete()` in its `onTopicCreated` hook, creating a topic, and verifying the AI-generated content appears in the topic card and timeline.

**Acceptance Scenarios**:

1. **Given** a Type Skill with an `onTopicCreated` hook that calls `AiService.complete()`, **When** a topic of that type is created, **Then** the AI service processes the request and returns a response that the Skill can use in its hook logic.
2. **Given** the AI service is configured and available, **When** a Skill calls `AiService.complete()` with a prompt, **Then** the response is returned within the configured timeout and contains the model's output text and token usage.
3. **Given** the AI service is unavailable (circuit breaker open), **When** a Skill calls `AiService.complete()`, **Then** the call returns `null` instead of throwing, and the Skill can handle the absence gracefully (e.g., skip AI-enhanced content, use a fallback).
4. **Given** a Skill that does NOT use AI, **When** a topic lifecycle event occurs, **Then** the Skill pipeline executes exactly as before — zero changes to non-AI Skills.
...
- **FR-001**: System MUST provide an `AiService` that any Skill can use to make AI model calls. The service MUST be injectable via NestJS dependency injection and available to all Skill categories (Type, Platform, Auth, Adapter).
- **FR-002**: `AiService.complete()` MUST accept a structured request (system prompt, user prompt, optional parameters) and return either an `AiResponse` (model output + token usage) or `null` (when AI is unavailable, disabled, or rate-limited).
...
- **SC-001**: A test-only Skill fixture that calls `AiService.complete()` in its `onTopicCreated` hook executes successfully and the AI-generated content appears in the topic timeline within 5 seconds
...
- The existing Skill pipeline architecture can accommodate AI as an injectable service without fundamental restructuring
