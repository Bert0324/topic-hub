...
export function resolveTsdownBuildInvocation(params = {}) {
  ...
  /** When vendored inside topic-hub, use workspace-root tsdown so rolldown native bindings resolve. */
  const tsdownNodeEntry =
    typeof env.OPENCLAW_TSDOWN_NODE_ENTRY === "string" ? env.OPENCLAW_TSDOWN_NODE_ENTRY.trim() : "";
  if (tsdownNodeEntry.length > 0 && fs.existsSync(tsdownNodeEntry)) {
    return {
      command: nodeExecPath,
      args: [tsdownNodeEntry, "--config-loader", "unrun", "--logLevel", logLevel, ...extraArgs],
      options: {
        encoding: "utf8",
        stdio: "pipe",
        shell: false,
