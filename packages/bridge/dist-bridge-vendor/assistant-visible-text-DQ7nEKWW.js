...
function stripToolCallXmlTags(text) {
	...
	for (let idx = 0; idx < text.length; idx += 1) {
		...
		if (!inToolCallBlock) {
			...
			const hasToolCallPayloadStart = tag.tagName === "tool_call" ? looksLikeToolCallPayloadStart(text, payloadStart) : TOOL_CALL_JSON_PAYLOAD_START_RE.test(text.slice(payloadStart));
		...
		} else if (tag.isClose && (tag.tagName === toolCallBlockTagName || toolCallBlockTagName === "tool_result" && tag.tagName === "tool_call") && !endsInsideQuotedString(text, toolCallContentStart, idx)) {
...
//#endregion
export { stripDowngradedToolCallText as a, stripReasoningTagsFromText as c, isInsideCode as d, stripAssistantInternalScaffolding as i, stripModelSpecialTokens as l, sanitizeAssistantVisibleTextWithOptions as n, stripMinimaxToolCallXml as o, sanitizeAssistantVisibleTextWithProfile as r, stripToolCallXmlTags as s, sanitizeAssistantVisibleText as t, findCodeRegions as u };
