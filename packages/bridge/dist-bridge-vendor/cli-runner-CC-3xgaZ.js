...
import { l as isRecord$2 } from "./utils-BpVTx0yp.js";
...
import { p as scopedHeartbeatWakeOptions } from "./session-key-CprbVBQX.js";
import { a as logWarn, t as logDebug } from "./logger-tSqm_V4O.js";
import { b as resolveSessionAgentIds } from "./agent-scope-D2A6iYD-.js";
import { a as loadConfig, at as resolveOwnerDisplaySetting } from "./io-CIh0_WgN.js";
import { n as resolvePluginSetupCliBackend } from "./setup-registry-D8Eid6bm.js";
import { r as normalizeProviderId } from "./provider-id-DDriY74A.js";
import { o as sanitizeHostExecEnv } from "./host-env-security-B0Rck7M4.js";
...
import { n as applySkillEnvOverridesFromSnapshot } from "./env-overrides-jnbPgR23.js";
...
import { n as detectRuntimeShell, t as getProcessSupervisor } from "./supervisor-CIAB88Ar.js";
import { i as resolveMainSessionKey } from "./main-session-Cg1RPT_G.js";
import { $ as resolveRuntimeTextTransforms, et as applyPluginTextReplacements, tt as mergePluginTextTransforms } from "./provider-runtime-ljZXzW8q.js";
import { d as extractMcpServerMap, p as loadEnabledBundleMcpConfig } from "./loader-B-B7CN7x.js";
import { b as readCodexCliCredentialsCached, o as loadAuthProfileStoreForRuntime, y as readClaudeCliCredentialsCached } from "./store-CJsXfApK.js";
import { C as resolveRuntimeCliBackends, f as resolveDefaultModelForAgent } from "./model-selection-DYo_mceY.js";
...
import { n as extensionForMime } from "./mime-BY3E3Z_p.js";
...
import { i as resolveBootstrapContextForRun, o as resolveHeartbeatPromptForSystemPrompt, r as makeBootstrapWarn } from "./bootstrap-files-BtMJRen2.js";
import { F as resolveBootstrapTotalMaxChars, N as resolveBootstrapMaxChars, P as resolveBootstrapPromptTruncationWarningMode, T as isFailoverErrorMessage, m as classifyFailoverReason } from "./pi-embedded-helpers-x9Tf7Zu1.js";
import { n as sanitizeImageBlocks } from "./tool-images-BG8bgK9t.js";
import { i as isFailoverError, s as resolveFailoverStatus, t as FailoverError } from "./failover-error-Dvvg9Pjq.js";
import { i as emitAgentEvent } from "./agent-events-L3WabTAF.js";
...
import { t as buildAgentSystemPrompt } from "./system-prompt-D83XxDVi.js";
import { a as loadImageFromRef, i as detectImageReferences, n as resolveRunWorkspaceDir, t as redactRunIdentifier } from "./workspace-run-CmVflHl3.js";
import { t as KeyedAsyncQueue } from "./keyed-async-queue-CbgIDdWc.js";
import { n as CLI_RESUME_WATCHDOG_DEFAULTS, r as CLI_WATCHDOG_MIN_TIMEOUT_MS, t as CLI_FRESH_WATCHDOG_DEFAULTS } from "./cli-watchdog-defaults-BBYgx_3M.js";
import { a as getHeader } from "./http-utils-B8zBCncg.js";
import { t as resolveGatewayScopedTools } from "./tool-resolution-CO2BcJbi.js";
...
function isClaudeCliProvider$1(providerId) {
	return normalizeLowercaseStringOrEmpty(providerId) === "claude-cli";
}
function usesClaudeStreamJsonDialect(params) {
	return params.backend.jsonlDialect === "claude-stream-json" || isClaudeCliProvider$1(params.providerId);
}
...
function parseJsonRecordCandidates(raw) {
	...
	for (const candidate of extractJsonObjectCandidates(trimmed)) try {
		...
		if (isRecord$2(parsed)) parsedRecords.push(parsed);
	} catch {}
	return parsedRecords;
}
function readNestedErrorMessage(parsed) {
	if (isRecord$2(parsed.error)) {
...
function toCliUsage(raw) {
	...
	const cacheRead = pick("cache_read_input_tokens") ?? pick("cached_input_tokens") ?? pick("cacheRead") ?? pick("cached");
	const input = pick("input") ?? (Object.hasOwn(raw, "cached") && typeof totalInput === "number" ? Math.max(0, totalInput - (cacheRead ?? 0)) : totalInput);
...
function readCliUsage(parsed) {
	if (isRecord$2(parsed.usage)) {
		const usage = toCliUsage(parsed.usage);
		if (usage) return usage;
	}
	if (isRecord$2(parsed.stats)) return toCliUsage(parsed.stats);
}
function collectCliText(value) {
	...
	if (isRecord$2(value.message)) return collectCliText(value.message);
	return "";
}
...
function parseClaudeCliStreamingDelta(params) {
	if (!usesClaudeStreamJsonDialect(params)) return null;
	if (params.parsed.type !== "stream_event" || !isRecord$2(params.parsed.event)) return null;
	const event = params.parsed.event;
	if (event.type !== "content_block_delta" || !isRecord$2(event.delta)) return null;
...
function createCliJsonlStreamingParser(params) {
	...
	const handleParsedRecord = (parsed) => {
		...
		if (isRecord$2(parsed.usage)) usage = toCliUsage(parsed.usage) ?? usage;
...
function parseCliJsonl(raw, backend, providerId) {
	...
	for (const line of lines) for (const parsed of parseJsonRecordCandidates(line)) {
		...
		const item = isRecord$2(parsed.item) ? parsed.item : null;
...
const CLAUDE_CLI_BACKEND_ID$1 = "claude-cli";
...
async function prepareClaudeCliSkillsPlugin(params) {
	if (normalizeLowercaseStringOrEmpty(params.backendId) !== CLAUDE_CLI_BACKEND_ID$1) return {
		args: [],
		cleanup: async () => {}
	};
...
function serializeTomlInlineValue(value) {
	...
	if (isRecord$1(value)) return `{ ${Object.entries(value).map(([key, entry]) => `${formatTomlKey(key)} = ${serializeTomlInlineValue(entry)}`).join(", ")} }`;
...
function resolveSystemPromptUsage(params) {
	...
	if (!params.backend.systemPromptArg?.trim() && !params.backend.systemPromptFileConfigKey?.trim()) return null;
	return systemPrompt;
}
...
async function writeCliSystemPromptFile(params) {
	if (!params.backend.systemPromptFileConfigKey?.trim()) return { cleanup: async () => {} };
...
function buildCliArgs(params) {
	...
	if (!params.useResume && params.systemPrompt && params.systemPromptFilePath && params.backend.systemPromptFileConfigKey) args.push(params.backend.systemPromptFileConfigArg ?? "-c", formatTomlConfigOverride(params.backend.systemPromptFileConfigKey, params.systemPromptFilePath));
	...
	if (params.promptArg !== void 0) {
		...
		if (replacedPromptPlaceholder) return args;
	...
	try {
		return await enqueueCliRun(queueKey, async () => {
			...
			try {
				...
				return {
					...parsed,
					text: applyPluginTextReplacements(parsed.text, context.backendResolved.textTransforms?.output)
				};
			} finally {
				restoreSkillEnv?.();
			}
...
function validateMcpLoopbackRequest(params) {
	...
	if ((getHeader(params.req, "authorization") ?? "") !== `Bearer ${params.token}`) {
...
const prepareDeps = {
	...
	resolveOpenClawDocsPath: async (params) => (await import("./docs-path-CdLdDIU1.js")).resolveOpenClawDocsPath(params)
};
...
//#endregion
export { setCliSessionBinding as a, getCliSessionId as i, clearCliSession as n, setCliSessionId as o, getCliSessionBinding as r, closeMcpLoopbackServer as s, runCliAgent as t };
