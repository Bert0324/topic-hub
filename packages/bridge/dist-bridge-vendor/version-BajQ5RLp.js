...
/** Package names whose `package.json` next to `dist/` defines the gateway semver (Topic Hub uses `@topichub/bridge`). */
const GATEWAY_PACKAGE_NAMES = new Set(["openclaw", "@topichub/bridge"]);
...
function readVersionFromJsonCandidates(moduleUrl, candidates, opts = {}) {
	try {
		...
		for (const candidate of candidates) try {
			...
			if (!version) continue;
			if (opts.requirePackageName && (!parsed.name || !GATEWAY_PACKAGE_NAMES.has(parsed.name))) continue;
			return version;
		} catch {}
		return null;
	} catch {
...
//#endregion
export { resolveBinaryVersion as a, resolveUsableRuntimeVersion as c, readVersionFromPackageJsonForModuleUrl as i, resolveVersionFromModuleUrl as l, VERSION as n, resolveCompatibilityHostVersion as o, readVersionFromBuildInfoForModuleUrl as r, resolveRuntimeServiceVersion as s, RUNTIME_SERVICE_VERSION_FALLBACK as t };
