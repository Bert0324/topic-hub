...
import { l as normalizeSecretInputString } from "./types.secrets-D88wZd3F.js";
import { u as resolveGatewayPort } from "./paths-D9v5qmRt.js";
import { h as writeConfigFile, l as readConfigFileSnapshot } from "./io-CIh0_WgN.js";
...
import { n as buildPluginCompatibilityNotices, s as formatPluginCompatibilityNotice } from "./status-dkQfSqE5.js";
import { t as resolveSetupSecretInputString } from "./setup.secret-input-BKOfJgpO.js";
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-UYLGG81j.js");
	...
	const matchedProvider = resolvedChoice?.provider ?? (preferredProvider ? providers.find((provider) => provider.id.trim() === preferredProvider.trim()) : void 0);
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-tpnUdxLg.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-CknHHufm.js");
		const { logConfigUpdated } = await import("./logging-DiMscgWi.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-qksV53PF.js");
	let nextConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
	const { ensureAuthProfileStore } = await import("./agents/auth-profiles.runtime.js");
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-CdrGB3E5.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-CMy-Pweu.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-Bgu0yIrI.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-Cgs0dWGp.js");
	...
	const authChoiceModelSelectionPolicy = authChoice === "custom-api-key" ? void 0 : await resolveAuthChoiceModelSelectionPolicy({
		authChoice,
		config: nextConfig,
		workspaceDir,
		resolvePreferredProviderForAuthChoice
	});
	if (authChoice !== "custom-api-key" && (authChoiceFromPrompt || authChoiceModelSelectionPolicy?.promptWhenAuthChoiceProvided === true)) {
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-j0I4XnX_.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-rszYSBn-.js");
		const { setupChannels } = await import("./onboard-channels-CzroZBvD.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-BDD4mPuF.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-D-1xjLyV.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupPluginConfig } = await import("./setup.plugin-config-mPAB_MBo.js");
	...
	const { setupInternalHooks } = await import("./onboard-hooks-BD_cUAEB.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-ujfFmAG4.js");
