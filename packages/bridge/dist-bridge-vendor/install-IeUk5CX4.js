...
import { m as resolveUserPath, t as CONFIG_DIR } from "./utils-BpVTx0yp.js";
import { a as resolvePackageExtensionEntries } from "./manifest-DVVfeFh-.js";
import { a as safePathSegmentHashed, i as safeDirName, n as packageNameMatchesId, o as unscopedPackageName, r as resolveSafeInstallDir } from "./install-safe-path-DZdzow0X.js";
import path from "node:path";
import fs from "node:fs/promises";
//#region src/plugins/install.ts
let pluginInstallRuntimePromise;
async function loadPluginInstallRuntime() {
	pluginInstallRuntimePromise ??= import("./install.runtime-D3Q6E1Pp.js");
	...
	return await installPluginDirectoryIntoExtensions({
		...
		afterCopy: async (installedDir) => {
			for (const entry of extensions) {
				...
				if (!await runtime.fileExists(resolvedEntry)) logger.warn?.(`extension entry not found: ${entry}`);
			}
		},
		afterInstall: async (installedDir) => await runInstallSourceScan({
			subject: `Plugin "${pluginId}"`,
...
//#endregion
export { resolvePluginInstallDir as a, installPluginFromPath as i, installPluginFromArchive as n, installPluginFromNpmSpec as r, PLUGIN_INSTALL_ERROR_CODE as t };
