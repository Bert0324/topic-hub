...
import { a as openVerifiedFileSync, f as isPathInside, i as openBoundaryFileSync } from "./boundary-file-read-4GSqtybS.js";
import { m as resolveUserPath, p as resolveHomeDir, t as CONFIG_DIR } from "./utils-BpVTx0yp.js";
import { _ as mergeBundlePathLists, f as CLAUDE_BUNDLE_MANIFEST_RELATIVE_PATH, v as normalizeBundlePathList } from "./ids-BhKtFNS_.js";
...
import { r as normalizeSkillFilter, t as resolveEffectiveAgentSkillFilter } from "./agent-filter-CtOfYR2u.js";
import { c as resolveEffectivePluginActivationState, l as resolveMemorySlotDecision, n as loadPluginManifestRegistry, s as normalizePluginsConfigWithResolver } from "./manifest-registry--_2x2rrY.js";
import { r as hasKind } from "./slots-YfKufJfg.js";
import { r as isPathInsideWithRealpath } from "./scan-paths-BsZgPBUU.js";
import { a as normalizePluginsConfig, s as resolveEffectivePluginActivationState$1 } from "./config-state-9vPQf81t.js";
import { i as resolveSandboxPath } from "./sandbox-paths-BpWrwuiS.js";
import { u as parseFrontmatterBlock } from "./frontmatter-D5ImIvbi.js";
import { d as resolveOpenClawMetadata, f as resolveSkillInvocationPolicy, l as shouldIncludeSkill, u as parseFrontmatter } from "./env-overrides-jnbPgR23.js";
...
function loadSkillsFromDirSafe(params) {
	...
	if (rootSkill) return { skills: [rootSkill] };
	return { skills: listCandidateSkillDirs(rootDir).map((skillDir) => loadSingleSkillDirectory({
		skillDir,
		source: params.source,
		rootRealPath,
		maxBytes: params.maxBytes
...
function resolveCompactHomePrefixes() {
	const resolvedHomes = [resolveHomeDir(), resolveUserHomeDir()].filter((home) => !!home).map((home) => path.resolve(home));
...
function filterSkillEntries(entries, config, skillFilter, eligibility) {
	...
	if (skillFilter !== void 0) {
		...
		skillsLogger$1.debug(`Applying skill filter: ${label}`);
		filtered = normalized.length > 0 ? filtered.filter((entry) => normalized.includes(entry.skill.name)) : [];
		skillsLogger$1.debug(`After skill filter: ${filtered.map((entry) => entry.skill.name).join(", ") || "(none)"}`);
	}
	return filtered;
}
...
function loadSkillEntries(workspaceDir, opts) {
	...
	const loadSkills = (params) => {
		...
		if (fs.existsSync(rootSkillMd)) {
			...
			try {
				...
				if (size > limits.maxSkillFileBytes) {
					skillsLogger$1.warn("Skipping skills root due to oversized SKILL.md.", {
						dir: baseDir,
			...
			return filterLoadedSkillsInsideRoot({
				skills: unwrapLoadedSkills(loadSkillsFromDirSafe({
					dir: baseDir,
					source: params.source,
		...
		if (suspicious) skillsLogger$1.warn("Skills root looks suspiciously large, truncating discovery.", {
			dir: params.dir,
			baseDir,
		...
		else if (childDirs.length > maxCandidates) skillsLogger$1.warn("Skills root has many entries, truncating discovery.", {
			dir: params.dir,
			baseDir,
			childDirCount: childDirs.length,
			maxSkillsLoadedPerSource: limits.maxSkillsLoadedPerSource
		...
		for (const name of limitedChildren) {
			...
			try {
				...
				if (size > limits.maxSkillFileBytes) {
					skillsLogger$1.warn("Skipping skill due to oversized SKILL.md.", {
						skill: name,
						filePath: skillMd,
						size,
						maxSkillFileBytes: limits.maxSkillFileBytes
	...
	await serializeByKey(`syncSkills:${targetDir}`, async () => {
		...
		for (const entry of entries) {
			...
			try {
				...
				skillsLogger$1.warn(`Failed to resolve safe destination for ${entry.skill.name}: ${message}`);
				continue;
			}
			if (!dest) {
				skillsLogger$1.warn(`Failed to resolve safe destination for ${entry.skill.name}: invalid source directory name`);
				continue;
			}
			try {
				...
				const message = error instanceof Error ? error.message : JSON.stringify(error);
				skillsLogger$1.warn(`Failed to copy ${entry.skill.name} to sandbox: ${message}`);
			}
		}
	});
}
...
function loadEnabledClaudeBundleCommands(params) {
	...
	for (const record of registry.plugins) {
		...
		if (!resolveEffectivePluginActivationState$1({
			id: record.id,
			origin: record.origin,
			config: normalizedPlugins,
...
//#endregion
export { filterWorkspaceSkillEntries as a, resolveSkillsPromptForRun as c, loadSkillsFromDirSafe as d, resolveBundledSkillsDir as f, buildWorkspaceSkillsPrompt as i, syncSkillsToWorkspace as l, buildWorkspaceSkillCommandSpecs as n, filterWorkspaceSkillEntriesWithOptions as o, buildWorkspaceSkillSnapshot as r, loadWorkspaceSkillEntries as s, resolveSkillsInstallPreferences as t, resolvePluginSkillDirs as u };
