...
import { a as normalizePluginIdScope, n as createPluginIdScopeSet, o as serializePluginIdScope } from "./channel-configured-jSiWbzTw.js";
import { t as resolveBundledPluginCompatibleActivationInputs } from "./activation-context-DKiXqKQn.js";
...
function resolveManifestDeclaredWebProviderCandidatePluginIds(params) {
	...
	const ids = loadPluginManifestRegistry({
		...
		workspaceDir: params.workspaceDir,
		env: params.env
	}).plugins.filter((plugin) => (!params.origin || plugin.origin === params.origin) && (!onlyPluginIdSet || onlyPluginIdSet.has(plugin.id)) && (contractIds.has(plugin.id) || pluginManifestDeclaresProviderConfig(plugin, params.configKey, params.contract))).map((plugin) => plugin.id).toSorted((left, right) => left.localeCompare(right));
