...
import { c as listChatChannels, d as isChannelVisibleInSetup, n as formatChannelSelectionLine, t as formatChannelPrimerLine } from "./registry-BRjSWImW.js";
import { n as CHAT_CHANNEL_ORDER } from "./ids-BhKtFNS_.js";
import { i as formatErrorMessage } from "./errors-BTh8VBsl.js";
import { s as listBundledChannelSetupPlugins } from "./bundled-CpwZ2fW5.js";
import { n as normalizeAccountId, t as DEFAULT_ACCOUNT_ID } from "./account-id-BMe6W-KZ.js";
...
import { t as isChannelConfigured } from "./channel-configured-jSiWbzTw.js";
import { t as applyPluginAutoEnable } from "./plugin-auto-enable-MDj5RYW_.js";
import { a as getActivePluginRegistryVersion, g as requireActivePluginRegistry } from "./runtime-DcBif8u5.js";
import { t as formatDocsLink } from "./links-Dnv0Os5x.js";
import { i as resolveChannelDefaultAccountId } from "./helpers-GunoyEwO.js";
import { B as runSingleChannelSecretStep, F as resolveAccountIdForConfigure, N as promptResolvedAllowFrom, Q as splitSetupEntries } from "./setup-wizard-helpers-D215gF3L.js";
import { t as promptChannelAccessConfig } from "./setup-group-access-BV64ldJS.js";
import { t as enablePluginInConfig } from "./enable-CawLDRmd.js";
import { i as listTrustedChannelPluginCatalogEntries, n as loadChannelSetupPluginRegistrySnapshotForChannel, r as listSetupDiscoveryChannelPluginCatalogEntries, t as ensureChannelSetupPluginInstalled } from "./plugin-install-CN2QALDX.js";
...
function resolveChannelSetupEntries(params) {
	...
	const installedCatalogEntries = installedCatalogEntriesSource.filter((entry) => !installedPluginIds.has(entry.id) && manifestInstalledIds.has(entry.id) && shouldShowChannelInSetup(entry.meta));
	const installableCatalogEntries = installableCatalogEntriesSource.filter((entry) => !installedPluginIds.has(entry.id) && !manifestInstalledIds.has(entry.id) && shouldShowChannelInSetup(entry.meta));
...
function resolveChannelSetupSelectionContributions(params) {
	return params.entries.filter((entry) => shouldShowChannelInSetup(entry.meta)).map((entry) => {
...
async function setupChannels(cfg, runtime, prompter, options) {
	...
	const listVisibleInstalledPlugins = () => {
		...
		for (const plugin of listChannelSetupPlugins()) if (shouldShowChannelInSetup(plugin.meta)) merged.set(plugin.id, plugin);
	...
	const corePrimer = listChatChannels().filter((meta) => shouldShowChannelInSetup(meta)).map((meta) => ({
		id: meta.id,
		label: meta.label,
		blurb: meta.blurb
	}));
...
//#endregion
export { noteChannelStatus as a, setupChannels as i, createChannelOnboardingPostWriteHookCollector as n, isCatalogChannelInstalled as o, runCollectedChannelOnboardingPostWriteHooks as r, shouldShowChannelInSetup as s, createChannelOnboardingPostWriteHook as t };
