...
import { _ as isAcpSessionKey, c as normalizeAgentId, l as normalizeMainKey, x as parseAgentSessionKey } from "./session-key-CprbVBQX.js";
import { n as resolveGlobalSingleton } from "./global-singleton-Bu7EzZLX.js";
import { i as resolveMainSessionKey, t as canonicalizeMainSessionAlias } from "./main-session-Cg1RPT_G.js";
import { a as failTaskRunByRunId, i as createRunningTaskRun, r as completeTaskRunByRunId, u as startTaskRunByRunId } from "./task-executor-DTBvc_JV.js";
import { t as resolveAgentTimeoutMs } from "./timeout-oGay3Wrd.js";
import { t as KeyedAsyncQueue } from "./keyed-async-queue-CbgIDdWc.js";
import { a as withAcpRuntimeErrorBoundary, i as toAcpRuntimeError, n as AcpRuntimeError, t as ACP_ERROR_CODES } from "./errors-DuZeqcVS.js";
import { a as identityHasStableSessionId, c as resolveRuntimeHandleIdentifiersFromIdentity, i as identityEquals, l as resolveRuntimeResumeSessionId, n as createIdentityFromHandleEvent, o as isSessionIdentityPending, r as createIdentityFromStatus, s as mergeSessionIdentity, t as createIdentityFromEnsure, u as resolveSessionIdentityFromMeta } from "./session-identity-BXwtPf0I.js";
import { i as upsertAcpSessionMeta, n as readAcpSessionEntry, t as listAcpSessionEntries } from "./session-meta-d6yt4YE8.js";
...
async function resolveManagerRuntimeCapabilities(params) {
	...
	const normalizedKeys = (reported?.configOptionKeys ?? []).map((entry) => normalizeOptionalString(entry)).filter(Boolean);
	return {
...
async function applyManagerRuntimeControls(params) {
	...
	const advertisedKeys = new Set((capabilities.configOptionKeys ?? []).map((entry) => normalizeOptionalString(entry)).filter(Boolean));
...
function resolveAcpRuntimeRegistryGlobalState() {
	return resolveGlobalSingleton(ACP_RUNTIME_REGISTRY_STATE_KEY, () => ({ backendsById: /* @__PURE__ */ new Map() }));
}
...
function appendBackgroundTaskProgressSummary(current, chunk) {
	const normalizedChunk = normalizeOptionalString(chunk)?.replace(/\s+/g, " ");
...
var AcpSessionManager = class {
	...
	async initializeSession(input) {
		...
		return await this.withSessionActor(sessionKey, async () => {
			...
			if (!persisted?.acp) {
				...
				throw new AcpRuntimeError("ACP_SESSION_INIT_FAILED", `Could not persist ACP metadata for ${sessionKey}.`);
			}
	...
	async runTurn(input) {
		...
		await this.withSessionActor(sessionKey, async () => {
			...
			const taskContext = input.mode === "prompt" ? this.resolveBackgroundTaskContext({
				cfg: input.cfg,
				sessionKey,
				requestId: input.requestId,
				text: input.text
			}) : null;
			if (taskContext) this.createBackgroundTaskRecord(taskContext, turnStartedAt);
				...
				try {
					...
					const turnPromise = (async () => {
						for await (const event of runtime.runTurn({
							...
							if (event.type === "error") streamError = new AcpRuntimeError(normalizeAcpErrorCode(event.code), normalizeOptionalString(event.message) || "ACP turn failed before completion.");
								...
								if (taskContext) this.markBackgroundTaskRunning(taskContext.runId, {
									sessionKey,
					...
					if (taskContext) this.markBackgroundTaskTerminal(taskContext.runId, {
						sessionKey,
		...
		return await this.withSessionActor(sessionKey, async () => {
			...
			if (shouldSkipRuntimeClose) {
				if (input.discardPersistentState) {
					...
					try {
						await this.deps.getRuntimeBackend(configuredBackend || void 0)?.runtime.prepareFreshSession?.({ sessionKey });
					} catch (error) {
						logVerbose(`acp close fast-reset: unable to prepare fresh session for ${sessionKey}: ${error instanceof Error ? error.message : String(error)}`);
					}
				}
				this.clearCachedRuntimeState(sessionKey);
			} else try {
				...
				if (input.allowBackendUnavailable && (acpError.code === "ACP_BACKEND_MISSING" || acpError.code === "ACP_BACKEND_UNAVAILABLE" || input.discardPersistentState && acpError.code === "ACP_SESSION_INIT_FAILED" || this.isRecoverableAcpxExitError(acpError.message))) {
					if (input.discardPersistentState) {
						const configuredBackend = (meta.backend || input.cfg.acp?.backend || "").trim();
						try {
							const runtimeBackend = this.deps.getRuntimeBackend(configuredBackend || void 0);
							if (!runtimeBackend) throw acpError;
							await runtimeBackend.runtime.prepareFreshSession?.({ sessionKey });
						} catch (recoveryError) {
							logVerbose(`acp close recovery: unable to prepare fresh session for ${sessionKey}: ${recoveryError instanceof Error ? recoveryError.message : String(recoveryError)}`);
						}
					}
					this.clearCachedRuntimeState(sessionKey);
			...
			if (input.discardPersistentState && !input.clearMeta) await this.discardPersistedRuntimeState({
				cfg: input.cfg,
				sessionKey
			});
			if (input.clearMeta) {
	...
	async ensureRuntimeHandle(params) {
		...
		if (cached) {
			...
			if (backendMatches && agentMatches && modeMatches && cwdMatches && handleMatchesMeta && await this.isCachedRuntimeHandleReusable({
				sessionKey: params.sessionKey,
	...
	async prepareFreshHandleRetry(params) {
		...
		if (!params.runtime || !params.meta || params.meta.mode !== "persistent" || !this.isRecoverableMissingPersistentSessionError(params.error.message)) return false;
		if (!await this.clearPersistedRuntimeResumeState({
			cfg: params.cfg,
			sessionKey: params.sessionKey
		})) return false;
	...
	isRecoverableMissingPersistentSessionError(message) {
	...
	async clearPersistedRuntimeResumeState(params) {
		const now = Date.now();
	...
	async discardPersistedRuntimeState(params) {
		const now = Date.now();
		await this.writeSessionMeta({
			cfg: params.cfg,
			sessionKey: params.sessionKey,
	...
	runtimeHandleMatchesMeta(params) {
	...
	resolveBackgroundTaskContext(params) {
		...
		return {
			requesterSessionKey,
			requesterOrigin: (this.deps.readSessionEntry({
				cfg: params.cfg,
	...
	createBackgroundTaskRecord(context, startedAt) {
		try {
	...
	markBackgroundTaskRunning(runId, params) {
		try {
			startTaskRunByRunId({
				runId,
				runtime: "acp",
	...
	markBackgroundTaskTerminal(runId, params) {
		try {
...
//#endregion
export { registerAcpRuntimeBackend as a, parseRuntimeTimeoutSecondsInput as c, validateRuntimeModeInput as d, validateRuntimeModelInput as f, getAcpRuntimeBackend as i, validateRuntimeConfigOptionInput as l, resolveAcpSessionResolutionError as m, getAcpSessionManager as n, requireAcpRuntimeBackend as o, validateRuntimePermissionProfileInput as p, __testing$1 as r, unregisterAcpRuntimeBackend as s, __testing as t, validateRuntimeCwdInput as u };
