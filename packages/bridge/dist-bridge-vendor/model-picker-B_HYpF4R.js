...
import { T as modelKey, d as resolveConfiguredModelRef, i as buildModelAliasIndex, t as buildAllowedModelSet } from "./model-selection-DYo_mceY.js";
import { t as resolveEnvApiKey } from "./model-auth-env-BwS_98pA.js";
import { o as hasUsableCustomProviderApiKey } from "./model-auth--xFDcZHC.js";
import { r as loadModelCatalog } from "./model-catalog-9wtELOal.js";
import { a as createLazyRuntimeSurface } from "./lazy-runtime-CL46Q9UY.js";
import { i as formatTokenK } from "./shared-CmSZxr0H.js";
...
async function loadModelPickerRuntime() {
	return import("./model-picker.runtime-BP0SK3KR.js");
}
...
function addModelSelectOption(params) {
	...
	if (params.seen.has(key) || HIDDEN_ROUTER_MODELS.has(key)) return;
	const hints = [];
...
async function maybeFilterModelsByProvider(params) {
	const providerIds = Array.from(new Set(params.models.map((entry) => entry.provider))).toSorted((a, b) => a.localeCompare(b));
	const hasPreferredProvider = !!params.preferredProvider;
	const shouldPromptProvider = !hasPreferredProvider && providerIds.length > 1 && params.models.length > PROVIDER_FILTER_THRESHOLD;
...
async function promptDefaultModel(params) {
	...
	const models = ignoreAllowlist ? catalog : (() => {
