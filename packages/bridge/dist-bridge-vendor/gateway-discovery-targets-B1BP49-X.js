...
import { t as isTailnetIPv4 } from "./tailnet-BFqEcob-.js";
...
function resolveGatewayDiscoveryEndpoint(beacon) {
	...
	if (!host || typeof port !== "number" || !Number.isFinite(port) || port <= 0) return null;
...
async function discoverGatewayBeacons(opts = {}) {
	...
	const domains = (domainsRaw.length > 0 ? domainsRaw : defaultDomains).map((d) => d.trim()).filter(Boolean).map((d) => d.endsWith(".") ? d : `${d}.`);
	try {
		if (platform === "darwin") {
...
//#endregion
export { discoverGatewayBeacons as i, buildGatewayDiscoveryTarget as n, serializeGatewayDiscoveryBeacon as r, buildGatewayDiscoveryLabel as t };
