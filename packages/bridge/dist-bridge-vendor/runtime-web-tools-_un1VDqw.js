...
import { l as isRecord } from "./utils-BpVTx0yp.js";
import { d as resolveSecretInputRef } from "./types.secrets-D88wZd3F.js";
import { u as secretRefKey } from "./ref-contract-aCgdDCZE.js";
import { h as resolveDefaultAgentId, m as resolveAgentWorkspaceDir } from "./agent-scope-D2A6iYD-.js";
import { a as collectPluginConfigContractMatches, o as resolvePluginConfigContractsById } from "./setup-registry-D8Eid6bm.js";
import { n as getBootstrapChannelSecrets } from "./bootstrap-registry-ClFoaYc7.js";
import { a as normalizePluginsConfig, c as resolveEnableState } from "./config-state-9vPQf81t.js";
import { t as loadBundledChannelSecretContractApi } from "./channel-contract-api-Ccfuo4ei.js";
import { n as normalizeSecretInput } from "./normalize-secret-input-DGmN5e8M.js";
import "./shared-CZLD4T6L.js";
import { o as resolveSecretRefValues } from "./resolve-CFv83bZp.js";
import { t as resolvePluginCapabilityProviders } from "./capability-provider-runtime-6ySqYPNa.js";
import { t as createGatewayCredentialPlan } from "./credential-planner-C7Kx1ZC7.js";
import { a as createLazyRuntimeSurface, i as createLazyRuntimeNamedExport } from "./lazy-runtime-CL46Q9UY.js";
import { n as resolveEffectiveMediaEntryCapabilities, r as normalizeMediaProviderId, t as resolveConfiguredMediaEntryCapabilities } from "./entry-capabilities-yFOFhDsq.js";
import { c as pushInactiveSurfaceWarning, l as pushWarning, n as collectSecretInputAssignment } from "./runtime-shared-BbBLyzMm.js";
import { t as collectTtsApiKeyAssignments } from "./runtime-config-collectors-tts-HH3rq6s6.js";
import { r as sortWebSearchProvidersForAutoDetect } from "./web-search-providers.shared-Bi0WA7YQ.js";
import { r as sortWebFetchProvidersForAutoDetect } from "./web-fetch-providers.shared-Dj7Ke-sq.js";
import { i as resolveBundledExplicitWebSearchProvidersFromPublicArtifacts, r as resolveBundledExplicitWebFetchProvidersFromPublicArtifacts } from "./web-provider-public-artifacts.explicit-Xnp2Wx0P.js";
...
function collectChannelConfigAssignments(params) {
	...
	for (const channelId of channelIds) (loadBundledChannelSecretContractApi(channelId)?.collectRuntimeConfigAssignments ?? getBootstrapChannelSecrets(channelId)?.collectRuntimeConfigAssignments)?.(params);
}
//#endregion
...
function buildMediaUnderstandingCapabilityRegistry(cfg) {
	...
	if (configProviders && typeof configProviders === "object") for (const [providerKey, providerCfg] of Object.entries(configProviders)) {
		...
		if (!(providerCfg.models ?? []).some((model) => Array.isArray(model?.input) && model.input.includes("image"))) continue;
...
const loadResolveManifestContractOwnerPluginId = createLazyRuntimeNamedExport(() => import("./runtime-web-tools-manifest.runtime-zFkao907.js"), "resolveManifestContractOwnerPluginId");
...
async function resolveRuntimeWebProviderSelection(params) {
	...
	if (params.enabled) {
		...
		for (const provider of candidates) {
			...
			if (params.configuredProvider) {
				...
				if (resolution.value) params.setResolvedCredential({
	...
	if (params.enabled && !params.configuredProvider && params.metadata.selectedProvider) for (const provider of params.providers) {
		if (provider.id === params.metadata.selectedProvider) continue;
		const value = params.readConfiguredCredential({
			provider,
			config: params.sourceConfig,
			toolConfig: params.toolConfig
		});
		if (!params.hasConfiguredSecretRef(value, params.defaults)) continue;
		for (const path of params.inactivePathsForProvider(provider)) pushInactiveSurfaceWarning({
			context: params.context,
			path,
			details: `${params.scopePath} auto-detected provider is "${params.metadata.selectedProvider}".`
		});
	}
	else if (params.toolConfig && !params.enabled) for (const provider of params.providers) {
		const value = params.readConfiguredCredential({
			provider,
			config: params.sourceConfig,
			toolConfig: params.toolConfig
	...
	if (params.enabled && params.toolConfig && params.configuredProvider) for (const provider of params.providers) {
		if (provider.id === params.configuredProvider) continue;
		const value = params.readConfiguredCredential({
			provider,
			config: params.sourceConfig,
			toolConfig: params.toolConfig
...
const loadRuntimeWebToolsFallbackProviders = createLazyRuntimeSurface(() => import("./runtime-web-tools-fallback.runtime-CFNagdZN.js"), ({ runtimeWebToolsFallbackProviders }) => runtimeWebToolsFallbackProviders);
const loadRuntimeWebToolsPublicArtifacts = createLazyRuntimeSurface(() => import("./runtime-web-tools-public-artifacts.runtime-rYycoSiD.js"), (mod) => mod);
const loadRuntimeWebToolsManifest = createLazyRuntimeSurface(() => import("./runtime-web-tools-manifest.runtime-zFkao907.js"), (mod) => mod);
...
async function resolveRuntimeWebTools(params) {
	...
	if (rawProvider && hasPluginWebSearchConfig) {
		...
		if (!configuredBundledWebSearchPluginIdHint && !await getHasCustomWebSearchRisk()) configuredBundledWebSearchPluginIdHint = inferSingleBundledPluginScopedWebToolConfigOwner(params.sourceConfig, "webSearch");
	}
	...
	if (search || hasPluginWebSearchConfig) {
		...
		if (!rawProvider && !hasPluginWebSearchConfig && isRecord(search) && Object.prototype.hasOwnProperty.call(search, "apiKey")) {
			const { resolveManifestContractPluginIdsByCompatibilityRuntimePath } = await loadRuntimeWebToolsManifest();
		...
		const searchSurface = await resolveRuntimeWebProviderSurface({
			...
			resolveProviders: async ({ configuredBundledPluginId }) => resolveBundledWebSearchProviders({
				...
				onlyPluginIds: configuredBundledPluginId === void 0 && searchCompatibilityOnlyPluginIds.length > 0 && !await getHasCustomWebSearchRisk() ? searchCompatibilityOnlyPluginIds : void 0,
...
//#endregion
export { evaluateGatewayAuthSurfaceStates as i, collectConfigAssignments as n, GATEWAY_AUTH_SURFACE_PATHS as r, resolveRuntimeWebTools as t };
