import { i as clampNumber, m as resolveUserPath, r as clampInt } from "./utils-BpVTx0yp.js";
import { _ as resolveStateDir } from "./paths-D9v5qmRt.js";
import { i as resolveAgentConfig } from "./agent-scope-D2A6iYD-.js";
import { a as isMemoryMultimodalEnabled, o as normalizeMemoryMultimodalSettings, s as supportsMemoryMultimodalEmbeddings } from "./multimodal-BgxgGY_l.js";
import { t as getMemoryEmbeddingProvider } from "./memory-embedding-provider-runtime-DcbTthfV.js";
...
function mergeConfig(defaults, overrides, agentId) {
	...
	const primaryAdapter = provider === "auto" ? void 0 : getMemoryEmbeddingProvider(provider);
	const defaultRemote = defaults?.remote;
	const overrideRemote = overrides?.remote;
	const fallback = overrides?.fallback ?? defaults?.fallback ?? "none";
	const fallbackAdapter = fallback && fallback !== "none" ? getMemoryEmbeddingProvider(fallback) : void 0;
	const includeRemote = Boolean(overrideRemote?.baseUrl || overrideRemote?.apiKey || overrideRemote?.headers || defaultRemote?.baseUrl || defaultRemote?.apiKey || defaultRemote?.headers) || provider === "auto" || primaryAdapter?.transport !== "local" || fallbackAdapter?.transport === "remote";
	const batch = {
...
function resolveMemorySearchConfig(cfg, agentId) {
	...
	const multimodalProvider = resolved.provider === "auto" ? void 0 : getMemoryEmbeddingProvider(resolved.provider);
	const builtinMultimodalSupport = resolved.provider === "auto" ? false : supportsMemoryMultimodalEmbeddings({
		provider: resolved.provider,
		model: resolved.model
	});
	if (multimodalActive && !(multimodalProvider?.supportsMultimodalEmbeddings?.({ model: resolved.model }) ?? builtinMultimodalSupport)) throw new Error("agents.*.memorySearch.multimodal requires a provider adapter that supports multimodal embeddings for the configured model.");
