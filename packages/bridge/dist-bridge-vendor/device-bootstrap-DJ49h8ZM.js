import { a as resolvePairingPaths, c as roleScopesAllow, n as verifyPairingToken, r as pruneExpiredPending, t as generatePairingToken } from "./pairing-token-CbaL-c7W.js";
import { n as readJsonFile, r as writeJsonAtomic, t as createAsyncLock } from "./json-files-CyG5Q-51.js";
import { r as normalizeDevicePublicKeyBase64Url } from "./device-identity-B57gmNDw.js";
import { n as normalizeDeviceBootstrapProfile, t as PAIRING_SETUP_BOOTSTRAP_PROFILE } from "./device-bootstrap-profile-kRtiNWgL.js";
...
function resolveBootstrapProfileScopes(role, scopes) {
	if (role === "operator") return scopes.filter((scope) => scope.startsWith("operator."));
...
function bootstrapProfileSatisfiesProfile(params) {
	for (const requiredRole of params.requiredProfile.roles) {
		...
		const requiredScopes = resolveBootstrapProfileScopes(requiredRole, params.requiredProfile.scopes);
...
async function redeemDeviceBootstrapTokenProfile(params) {
	return await withLock(async () => {
		...
		const redeemedProfile = normalizeDeviceBootstrapProfile({
			...
			scopes: [...resolvePersistedRedeemedProfile(record).scopes, ...resolveBootstrapProfileScopes(params.role, params.scopes)]
		});
		state[tokenKey] = {
			...record,
