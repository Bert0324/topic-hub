...
import { c as moveSingleAccountChannelSectionToDefaultAccount, l as patchScopedAccountConfig } from "./setup-helpers-CTezBbeW.js";
import { t as resolveSecretInputModeForEnvSelection } from "./provider-auth-mode-CpY1Fsy3.js";
//#region src/channels/plugins/setup-wizard-helpers.ts
let providerAuthInputPromise;
function loadProviderAuthInput() {
	providerAuthInputPromise ??= import("./provider-auth-ref-CC4Sejjt.js");
...
function createCompatChannelDmPolicy(params) {
	return {
		...
		getCurrent: (cfg, accountId) => {
			...
			const accountConfig = accountId && accountId !== "default" ? channelConfig.accounts?.[accountId] : void 0;
			return accountConfig?.dmPolicy ?? accountConfig?.dm?.policy ?? channelConfig.dmPolicy ?? channelConfig.dm?.policy ?? "pairing";
		},
		setPolicy: (cfg, policy, accountId) => accountId && accountId !== "default" ? patchChannelConfigForAccount({
			...
			patch: {
				dmPolicy: policy,
				...policy === "open" ? { allowFrom: addWildcardAllowFrom((cfg.channels?.[params.channel])?.accounts?.[accountId]?.allowFrom ?? (cfg.channels?.[params.channel])?.allowFrom ?? (cfg.channels?.[params.channel])?.accounts?.[accountId]?.dm?.allowFrom ?? (cfg.channels?.[params.channel])?.dm?.allowFrom) } : {}
			}
...
function createNestedChannelParsedAllowFromPrompt(params) {
	...
	if (typeof defaultAccountId === "function") return createPromptParsedAllowFromForAccount({
		defaultAccountId,
		...sharedParams
	});
	return createPromptParsedAllowFromForAccount({
...
const setLegacyChannelDmPolicyWithAllowFrom = setCompatChannelDmPolicyWithAllowFrom;
const createLegacyCompatChannelDmPolicy = createCompatChannelDmPolicy;
//#endregion
export { promptLegacyChannelAllowFromForAccount as A, runSingleChannelSecretStep as B, parseSetupEntriesAllowingWildcard as C, patchTopLevelChannelConfigSection as D, patchNestedChannelConfigSection as E, resolveAccountIdForConfigure as F, setLegacyChannelDmPolicyWithAllowFrom as G, setAccountDmAllowFromForChannel as H, resolveEntriesWithOptionalToken as I, setSetupChannelEnabled as J, setNestedChannelAllowFrom as K, resolveGroupAllowlistWithLookupNotes as L, promptParsedAllowFromForScopedChannel as M, promptResolvedAllowFrom as N, promptAccountId as O, promptSingleChannelSecretInput as P, splitSetupEntries as Q, resolveParsedAllowFromEntries as R, parseMentionOrPrefixedId as S, patchChannelConfigForAccount as T, setAccountGroupPolicyForChannel as U, setAccountAllowFromForChannel as V, setChannelDmPolicyWithAllowFrom as W, setTopLevelChannelDmPolicyWithAllowFrom as X, setTopLevelChannelAllowFrom as Y, setTopLevelChannelGroupPolicy as Z, createTopLevelChannelParsedAllowFromPrompt as _, createAllowFromSection as a, noteChannelLookupFailure as b, createNestedChannelDmPolicy as c, createPromptParsedAllowFromForAccount as d, createStandardChannelSetupStatus as f, createTopLevelChannelGroupPolicySetter as g, createTopLevelChannelDmPolicySetter as h, createAccountScopedGroupAccessSection as i, promptParsedAllowFromForAccount as j, promptLegacyChannelAllowFrom as k, createNestedChannelDmPolicySetter as l, createTopLevelChannelDmPolicy as m, buildSingleChannelSecretPromptState as n, createLegacyCompatChannelDmPolicy as o, createTopLevelChannelAllowFromSetter as p, setNestedChannelDmPolicyWithAllowFrom as q, createAccountScopedAllowFromSection as r, createNestedChannelAllowFromSetter as s, addWildcardAllowFrom as t, createNestedChannelParsedAllowFromPrompt as u, mergeAllowFromEntries as v, parseSetupEntriesWithParser as w, noteChannelLookupSummary as x, normalizeAllowFromEntries as y, resolveSetupAccountId as z };
