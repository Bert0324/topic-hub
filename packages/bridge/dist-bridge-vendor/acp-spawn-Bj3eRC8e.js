...
import { b as isSubagentSessionKey, c as normalizeAgentId, p as scopedHeartbeatWakeOptions, u as resolveAgentIdFromSessionKey, x as parseAgentSessionKey } from "./session-key-CprbVBQX.js";
import { h as resolveDefaultAgentId, i as resolveAgentConfig } from "./agent-scope-D2A6iYD-.js";
...
import { n as requestHeartbeatNow, t as areHeartbeatsEnabled } from "./heartbeat-wake-DlLYbFOv.js";
import { i as normalizeDeliveryContext, t as deliveryContextFromSession } from "./delivery-context.shared-DQgMr1Hj.js";
import { i as enqueueSystemEvent } from "./system-events-CKBbXDel.js";
import "./plugins-C60yWwZy.js";
import { n as resolveConversationDeliveryTarget, t as formatConversationTarget } from "./delivery-context-DfqDfkC4.js";
...
import { a as normalizeAssistantPhase } from "./chat-message-content-DJIhavaZ.js";
import "./heartbeat-DhJX9hg6.js";
...
import { i as createRunningTaskRun, s as recordTaskRunProgressByRunId } from "./task-executor-DTBvc_JV.js";
import { i as normalizeConversationTargetRef } from "./conversation-id-D6QjvxNL.js";
import { i as isSessionBindingError, r as getSessionBindingService } from "./session-binding-service-BIUfTe38.js";
import { t as resolveConversationIdFromTargets } from "./conversation-id-CsfFqPU-.js";
import { i as resolveSpawnedWorkspaceInheritance } from "./spawned-context-CdtiT-7T.js";
import { n as getAcpSessionManager } from "./manager-B3iT25IH.js";
import { n as readAcpSessionEntry } from "./session-meta-d6yt4YE8.js";
import { i as resolveThreadBindingThreadName, r as resolveThreadBindingIntroText } from "./thread-bindings-messages-BOTTpwlk.js";
import { d as resolveThreadBindingSpawnPolicy, l as resolveThreadBindingMaxAgeMsForChannel, n as formatThreadBindingSpawnDisabledError, o as resolveThreadBindingIdleTimeoutMsForChannel, t as formatThreadBindingDisabledError } from "./thread-bindings-policy-C3UQMr4L.js";
import { n as resolveAcpAgentPolicyError, t as isAcpEnabledByPolicy } from "./policy-CK_HUi46.js";
import { r as resolveAcpThreadSessionDetailLines, t as resolveAcpSessionCwd } from "./session-identifiers-DR3Kb_6F.js";
...
function resolvePlacementWithoutChannelPlugin(params) {
	switch (params.channel) {
		case "discord":
		case "matrix": return params.capabilities.placements.includes("child") ? "child" : "current";
		case "line":
		case "telegram": return "current";
	}
	return params.capabilities.placements.includes("child") ? "child" : "current";
}
function normalizeLineConversationIdFallback(value) {
...
function normalizeTelegramConversationIdFallback(params) {
	...
	const explicitThreadId = params.threadId != null ? normalizeOptionalString(String(params.threadId)) : void 0;
	if (explicitGroupId && explicitThreadId && /^-?\d+$/.test(explicitGroupId) && /^\d+$/.test(explicitThreadId)) return `${explicitGroupId}:topic:${explicitThreadId}`;
	...
	if (topicMatch?.[1] && topicMatch[2]) return `${topicMatch[1]}:topic:${topicMatch[2]}`;
	return /^-?\d+$/.test(normalized) ? normalized : void 0;
}
const threadBindingFallbackConversationResolvers = {
	line: (params) => normalizeLineConversationIdFallback(params.groupId ?? params.to),
	telegram: (params) => normalizeTelegramConversationIdFallback(params)
};
function resolvePluginConversationRefForThreadBinding(params) {
	const resolvedConversation = getChannelPlugin(params.channelId)?.messaging?.resolveInboundConversation?.({
		to: params.to,
...
function resolveConversationRefForThreadBinding(params) {
	...
	const compatibilityConversationId = channelKey && Object.hasOwn(threadBindingFallbackConversationResolvers, channelKey) ? threadBindingFallbackConversationResolvers[channelKey](params) : void 0;
	if (compatibilityConversationId) return normalizeConversationTargetRef({ conversationId: compatibilityConversationId });
...
function prepareAcpThreadBinding(params) {
	...
	const placementToUse = getChannelPlugin(policy.channel)?.conversationBindings?.defaultTopLevelPlacement ?? resolvePlacementWithoutChannelPlugin({
		channel: policy.channel,
		capabilities
	});
