...
import { i as loadPluginManifest, r as getPackageManifestMetadata } from "./manifest-DVVfeFh-.js";
import { r as buildChannelConfigSchema } from "./config-schema-2bWEw9uP.js";
import { l as resolveLoaderPackageRoot, n as buildPluginLoaderJitiOptions, u as resolvePluginLoaderJitiConfig } from "./sdk-alias-DQeTRBMb.js";
...
function isTopLevelPublicSurfaceSource(name) {
	if (!PUBLIC_SURFACE_SOURCE_EXTENSIONS$1.includes(path.extname(name))) return false;
...
function collectBundledPluginMetadataForPackageRoot(packageRoot, includeChannelConfigs, includeSyntheticChannelConfigs) {
	...
	const entries = [];
	for (const dirName of fs.readdirSync(scanDir, { withFileTypes: true }).filter((entry) => entry.isDirectory()).map((entry) => entry.name).toSorted((left, right) => left.localeCompare(right))) {
...
function resolveBundledPluginGeneratedPath(rootDir, entry, pluginDirName) {
	if (!entry) return null;
	const entryOrder = [entry.built, entry.source].filter((candidate) => typeof candidate === "string" && candidate.length > 0);
	const baseDirs = [path.resolve(rootDir, "dist", "extensions", pluginDirName ?? ""), path.resolve(rootDir, "extensions", pluginDirName ?? "")];
...
function resolveBundledPluginRepoEntryPath(params) {
	...
	const baseDirs = [path.resolve(params.rootDir, "dist", "extensions", metadata.dirName), path.resolve(params.rootDir, "extensions", metadata.dirName)];
	for (const baseDir of baseDirs) for (const entryPath of entryOrder) {
		const candidate = path.resolve(baseDir, normalizeRelativePluginEntryPath(entryPath));
		if (fs.existsSync(candidate)) return candidate;
	}
