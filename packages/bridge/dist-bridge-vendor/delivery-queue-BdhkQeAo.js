...
async function drainPendingDeliveries(opts) {
	...
	try {
		...
		const matchingEntries = (await loadPendingDeliveries(opts.stateDir)).map((entry) => ({
			entry,
			decision: opts.selectEntry(entry, now)
		})).filter((item) => item.decision.match).toSorted((a, b) => a.entry.enqueuedAt - b.entry.enqueuedAt);
...
//#endregion
export { recoverPendingDeliveries as a, ensureQueueDir as c, loadPendingDelivery as d, moveToFailed as f, isPermanentDeliveryError as i, failDelivery as l, drainPendingDeliveries as n, ackDelivery as o, isEntryEligibleForRecoveryRetry as r, enqueueDelivery as s, computeBackoffMs as t, loadPendingDeliveries as u };
