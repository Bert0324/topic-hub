...
import { r as normalizeControlUiBasePath } from "./control-ui-shared-B_UogItD.js";
import { t as buildGatewayConnectionDetailsWithResolvers } from "./connection-details-x4l0UETg.js";
import { t as createEmptyTaskRegistrySummary } from "./task-registry.summary-B-el_8bQ.js";
import { t as pickGatewaySelfPresence } from "./gateway-presence-D1e1z-hs.js";
import { t as createEmptyTaskAuditSummary } from "./task-registry.audit.shared-SVY7gIbA.js";
...
function loadGatewayProbeModule() {
	gatewayProbeModulePromise ??= import("./status.gateway-probe-0n70N5ey.js");
	return gatewayProbeModulePromise;
}
function loadProbeGatewayModule() {
	probeGatewayModulePromise ??= import("./probe-BMYJXogA.js");
...
async function resolveGatewayProbeSnapshot(params) {
	...
	const gatewayProbe = shouldProbe ? await loadProbeGatewayModule().then(({ probeGateway }) => probeGateway({
...
function loadStatusScanDepsRuntimeModule() {
	statusScanDepsRuntimeModulePromise ??= import("./status.scan.deps.runtime-DFh5j8WE.js");
	return statusScanDepsRuntimeModulePromise;
}
function loadStatusAgentLocalModule() {
	statusAgentLocalModulePromise ??= import("./status.agent-local-DVUvLycM.js");
	return statusAgentLocalModulePromise;
}
function loadStatusUpdateModule() {
	statusUpdateModulePromise ??= import("./status.update-BJKfuXur.js");
	return statusUpdateModulePromise;
}
function loadStatusScanRuntimeModule() {
	statusScanRuntimeModulePromise ??= import("./status.scan.runtime-D3jsATW8.js");
	return statusScanRuntimeModulePromise;
}
function loadGatewayCallModule() {
	gatewayCallModulePromise ??= import("./call-DlQ8xI7I.js");
	return gatewayCallModulePromise;
}
function loadStatusSummaryModule() {
	statusSummaryModulePromise ??= import("./status.summary-ClILEWj9.js");
...
function loadCommandConfigResolutionModule() {
	commandConfigResolutionModulePromise ??= import("./command-config-resolution-C5hjlzkI.js");
	return commandConfigResolutionModulePromise;
}
function loadCommandSecretTargetsModule() {
	commandSecretTargetsModulePromise ??= import("./command-secret-targets-CszfzQu_.js");
...
async function collectStatusScanOverview(params) {
	...
	const hasConfiguredChannels = params.resolveHasConfiguredChannels ? params.resolveHasConfiguredChannels(cfg) : hasPotentialConfiguredChannels(cfg);
	...
	const { channelsStatus, channelIssues, channels } = params.includeChannelsData !== false ? await (async () => {
