...
import { a as requireGatewayClientScopeForInternalChannel } from "./command-gates-C_tXS2Zv.js";
import { C as stopWithText, b as resolveAcpAction, d as COMMAND, x as resolveAcpHelpText } from "./shared-CHqdKfad.js";
...
async function loadAcpActionHandler(action) {
	if (action === "spawn" || action === "cancel" || action === "steer" || action === "close") {
		lifecycleHandlersPromise ??= import("./lifecycle-CqQjvY7U.js");
	...
	if (action === "status" || action === "set-mode" || action === "set" || action === "cwd" || action === "permissions" || action === "timeout" || action === "model" || action === "reset-options") {
		runtimeOptionHandlersPromise ??= import("./runtime-options-GhcdFJnI.js");
	...
	diagnosticHandlersPromise ??= import("./diagnostics-Cq5-lJga.js");
