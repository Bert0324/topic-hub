...
function resolveShellWrapperSpecAndArgvInternal(argv, depth) {
	if (!isWithinDispatchClassificationDepth(depth)) return null;
	const token0 = argv[0]?.trim();
	...
	if (dispatchUnwrap.kind === "unwrapped") return resolveShellWrapperSpecAndArgvInternal(dispatchUnwrap.argv, depth + 1);
	...
	if (shellMultiplexerUnwrap.kind === "unwrapped") return resolveShellWrapperSpecAndArgvInternal(shellMultiplexerUnwrap.argv, depth + 1);
...
function extractShellWrapperCommandInternal(argv, rawCommand, depth) {
	...
	return {
		isWrapper: true,
		command: rawCommand ?? resolved.payload
	};
}
...
function matchAllowlist(entries, resolution, argv, platform) {
	...
	const useArgPattern = normalizeLowercaseStringOrEmpty(platform ?? process.platform).startsWith("win");
...
//#endregion
export { POSIX_INLINE_COMMAND_FLAGS as A, resolveSafeBinProfiles as B, POWERSHELL_WRAPPERS as C, isShellWrapperExecutable as D, hasEnvManipulationBeforeShellWrapper as E, unwrapKnownDispatchWrapperInvocation as F, normalizeExecutableToken as I, DEFAULT_SAFE_BINS as L, resolveInlineCommandMatch as M, isDispatchWrapperExecutable as N, resolveShellWrapperTransportArgv as O, unwrapDispatchWrappersForResolution as P, SAFE_BIN_PROFILES as R, POSIX_SHELL_WRAPPERS as S, extractShellWrapperInlineCommand as T, resolvePolicyTargetCandidatePath as _, validateSafeBinArgv as a, resolveExecutablePath as b, matchAllowlist as c, resolveApprovalAuditCandidatePath as d, resolveCommandResolution as f, resolvePolicyAllowlistCandidatePath as g, resolveExecutionTargetResolution as h, normalizeTrustedSafeBinDirs as i, POWERSHELL_INLINE_COMMAND_FLAGS as j, unwrapKnownShellMultiplexerInvocation as k, parseExecArgvToken as l, resolveExecutionTargetCandidatePath as m, isTrustedSafeBinPath as n, listRiskyConfiguredSafeBins as o, resolveCommandResolutionFromArgv as p, listWritableExplicitTrustedSafeBinDirs as r, normalizeSafeBinName as s, getTrustedSafeBinDirs as t, resolveAllowlistCandidatePath as u, resolvePolicyTargetResolution as v, extractShellWrapperCommand as w, resolveExecWrapperTrustPlan as x, resolveExecutableFromPathEnv as y, normalizeSafeBinProfileFixtures as z };
