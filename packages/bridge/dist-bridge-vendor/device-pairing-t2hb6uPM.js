import { a as resolvePairingPaths, c as roleScopesAllow, i as reconcilePendingPairingRequests, n as verifyPairingToken, o as resolveMissingRequestedScope, r as pruneExpiredPending, s as resolveScopeOutsideRequestedRoles, t as generatePairingToken } from "./pairing-token-CbaL-c7W.js";
import { n as readJsonFile, r as writeJsonAtomic, t as createAsyncLock } from "./json-files-CyG5Q-51.js";
import { t as rejectPendingPairingRequest } from "./pairing-pending-Cdxr6_hK.js";
...
async function approveDevicePairing(requestId, optionsOrBaseDir, maybeBaseDir) {
	...
	return await withLock(async () => {
		...
		const requestedOperatorScopes = requestedScopes.filter((scope) => scope.startsWith(OPERATOR_SCOPE_PREFIX));
...
async function approveBootstrapDevicePairing(requestId, bootstrapProfile, baseDir) {
	...
	return await withLock(async () => {
		...
		const nextApprovedScopes = mergeScopes(existing?.approvedScopes ?? existing?.scopes, pending.scopes, approvedScopes);
		const tokens = existing?.tokens ? { ...existing.tokens } : {};
...
//#endregion
export { verifyDeviceToken as _, getPairedDevice as a, listDevicePairing as c, removePairedDevice as d, requestDevicePairing as f, updatePairedDeviceMetadata as g, summarizeDeviceTokens as h, formatDevicePairingForbiddenMessage as i, listEffectivePairedDeviceRoles as l, rotateDeviceToken as m, approveDevicePairing as n, hasEffectivePairedDeviceRole as o, revokeDeviceToken as p, ensureDeviceToken as r, listApprovedPairedDeviceRoles as s, approveBootstrapDevicePairing as t, rejectDevicePairing as u };
