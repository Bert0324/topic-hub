import { n as getPath, r as setPathCreateStrict } from "./path-utils-CplzGJxC.js";
	...
	for (const [key, hint] of Object.entries(plugin.uiHints)) {
		...
		if (schemaProp?.type === "array") {
			...
			if (trimmed !== currentStr) {
				if (trimmed) setPathCreateStrict(updatedConfig, pathSegments, trimmed.split(",").map((v) => v.trim()).filter(Boolean));
...
async function setupPluginConfig(params) {
	const { loadPluginManifestRegistry } = await import("./manifest-registry-DdZzl9TB.js");
	const unconfigured = discoverUnconfiguredPlugins({
		manifestPlugins: loadPluginManifestRegistry({
...
async function configurePluginConfig(params) {
	const { loadPluginManifestRegistry } = await import("./manifest-registry-DdZzl9TB.js");
	const configurable = discoverConfigurablePlugins({ manifestPlugins: loadPluginManifestRegistry({
		config: params.config,
		workspaceDir: params.workspaceDir
	}).plugins.filter((p) => {
		const entry = params.config.plugins?.entries?.[p.id];
...
//#endregion
export { configurePluginConfig, setupPluginConfig };
