import { p as resolvePinnedHostname } from "./ssrf-BYiMTIR_.js";
import { s as normalizeOptionalString } from "./string-coerce-D8LAEut5.js";
import { f as resolveConfigDir } from "./utils-BpVTx0yp.js";
import { t as retainSafeHeadersForCrossOriginRedirect } from "./redirect-headers-NTUm9oPv.js";
import { n as extensionForMime, t as detectMime } from "./mime-BY3E3Z_p.js";
import { d as readLocalFileSafely$1, t as SafeOpenError } from "./fs-safe-Co_6GYrb.js";
		...
		resolvePinnedHostnameImpl(parsedUrl.hostname).then((pinned) => {
			const req = requestImpl(parsedUrl, {
				...
				if (res.statusCode && res.statusCode >= 300 && res.statusCode < 400) {
					...
					resolve(downloadToFile(redirectUrl, dest, new URL(redirectUrl).origin === parsedUrl.origin ? headers : retainSafeHeadersForCrossOriginRedirect(headers), maxRedirects - 1));
					return;
				}
...
async function saveMediaSource(source, headers, subdir = "") {
	...
	try {
		...
		if (isSafeOpenError(err)) throw toSaveMediaSourceError(err);
		throw err;
	}
}
...
async function resolveMediaBufferPath(id, subdir = "inbound") {
	...
	if (!resolved.startsWith(dir + path.sep) && resolved !== dir) throw new Error(`resolveMediaBufferPath: path escapes media directory: ${JSON.stringify(id)}`);
	const stat = await fs$1.lstat(resolved);
...
async function deleteMediaBuffer(id, subdir = "inbound") {
	...
	await fs$1.unlink(physicalPath);
}
//#endregion
export { ensureMediaDir as a, resolveMediaBufferPath as c, setMediaStoreNetworkDepsForTest as d, deleteMediaBuffer as i, saveMediaBuffer as l, SaveMediaSourceError as n, extractOriginalFilename as o, cleanOldMedia as r, getMediaDir as s, MEDIA_MAX_BYTES as t, saveMediaSource as u };
