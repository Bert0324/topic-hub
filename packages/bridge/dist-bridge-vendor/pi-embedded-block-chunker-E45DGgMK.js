import { n as isSafeFenceBreak, r as parseFenceSpans, t as findFenceSpanAt } from "./fences-DJIUkd1X.js";
...
var EmbeddedBlockChunker = class {
	...
	drain(params) {
		...
		while (start < source.length) {
			...
			const breakResult = force && remainingLength <= maxChars ? this.#pickSoftBreakIndex(view, fenceSpans, 1, start) : this.#pickBreakIndex(view, fenceSpans, force ? 1 : void 0, start);
	...
	#emitBreakResult(params) {
		...
		emit(rawChunk);
		if (fenceSplit) return {
			start: absoluteBreakIdx,
			reopenFence: fenceSplit.fence
		};
		return {
			start: skipLeadingNewlines(source, absoluteBreakIdx < source.length && /\s/.test(source[absoluteBreakIdx]) ? absoluteBreakIdx + 1 : absoluteBreakIdx),
			reopenFence: void 0
		};
	}
	...
	#pickBreakIndex(buffer, fenceSpans, minCharsOverride, offset = 0) {
		const minChars = Math.max(1, Math.floor(minCharsOverride ?? this.#chunking.minChars));
		const maxChars = Math.max(minChars, Math.floor(this.#chunking.maxChars));
		if (buffer.length < minChars) return { index: -1 };
		...
		for (let i = window.length - 1; i >= minChars; i--) if (/\s/.test(window[i]) && isSafeFenceBreak(fenceSpans, offset + i)) return { index: i };
		if (buffer.length >= maxChars) {
			if (isSafeFenceBreak(fenceSpans, offset + maxChars)) return { index: maxChars };
			const fence = findFenceSpanAt(fenceSpans, offset + maxChars);
			if (fence) {
				const closeFenceStart = findFenceCloseLineStart(buffer, fence, offset);
				if (closeFenceStart >= minChars && closeFenceStart < maxChars) return {
