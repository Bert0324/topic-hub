...
/** Topic Hub `build:bridge-vendor`: skip compiling every `extensions/*` package (sparse checkouts). */
const isBridgeVendorBuild = process.env.OPENCLAW_BUILD_BRIDGE_VENDOR === "1";
...
/** Skip missing paths (sparse checkouts / vendored trees) so tsdown does not fail on UNRESOLVED_ENTRY. */
function bundledPluginEntriesIfPresent(
  pluginId: string,
  pairs: Array<{ entryKey: string; relativePath: string }>,
): Record<string, string> {
  const cwd = process.cwd();
  const out: Record<string, string> = {};
  for (const { entryKey, relativePath } of pairs) {
    const abs = path.join(cwd, "extensions", pluginId, ...relativePath.split("/"));
    if (fs.existsSync(abs)) {
      out[entryKey] = bundledPluginFile(pluginId, relativePath);
    }
  }
  return out;
}
...
function buildCoreDistEntries(): Record<string, string> {
  return {
    ...(isBridgeVendorBuild
      ? {
          "gateway/embed-export": "src/gateway/embed-export.ts",
          // Lazy-loaded by `plugin-sdk/tts-runtime` via bundled public surface; full `extensions/*`
          // graph is omitted in bridge-vendor builds, so compile this explicitly.
    ...
    ...bundledPluginEntriesIfPresent("telegram", [
      { entryKey: "telegram/audit", relativePath: "src/audit.ts" },
      { entryKey: "telegram/token", relativePath: "src/token.ts" },
    ]),
...
function buildUnifiedDistEntries(): Record<string, string> {
  return {
    ...
    ...(isBridgeVendorBuild ? {} : bundledPluginBuildEntries),
    ...bundledHookEntries,
  };
}
