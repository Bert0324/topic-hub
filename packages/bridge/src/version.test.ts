...
describe("version resolution", () => {
  ...
  it("accepts @topichub/bridge package name for vendored Topic Hub layout", async () => {
    await withTempDir({ prefix: "bridge-version-" }, async (root) => {
      await writeJsonFixture(root, "package.json", { name: "@topichub/bridge", version: "2026.4.12-beta.1" });
      const moduleUrl = await ensureModuleFixture(root);
      expect(readVersionFromPackageJsonForModuleUrl(moduleUrl)).toBe("2026.4.12-beta.1");
      expect(resolveVersionFromModuleUrl(moduleUrl)).toBe("2026.4.12-beta.1");
    });
  });
