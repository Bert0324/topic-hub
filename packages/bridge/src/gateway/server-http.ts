...
export function normalizeGatewayEmbedPathPrefix(pathPrefix: string): string {
...
export function stripGatewayEmbedPathPrefixFromUrl(
  rawUrl: string,
  pathPrefix: string,
): { pathname: string; search: string; matched: boolean } {
  ...
  try {
    ...
    return { pathname: "/", search: "", matched: false };
  }
  ...
  const rest =
    pathname.length === prefix.length
      ? "/"
      : pathname.slice(prefix.length).startsWith("/")
        ? pathname.slice(prefix.length)
        : `/${pathname.slice(prefix.length)}`;
  return { pathname: rest, search, matched: true };
}

export function wrapGatewayHttpListenerWithPathPrefix(
  pathPrefix: string,
  inner: (req: IncomingMessage, res: ServerResponse) => void,
  ...
  return (req, res) => {
    ...
    const raw = req.url ?? "/";
    const { pathname, search, matched } = stripGatewayEmbedPathPrefixFromUrl(raw, pathPrefix);
    if (!matched) {
      return;
    }
...
export type GatewayHttpRequestListenerOptions = {
 ...
 */
export function createAsyncGatewayHttpRequestHandler(
  opts: GatewayHttpRequestListenerOptions,
...
export function createGatewayHttpRequestListener(
  opts: GatewayHttpRequestListenerOptions,
): (req: IncomingMessage, res: ServerResponse) => void {
...
export function attachGatewayUpgradeHandler(opts: {
  ...
  /** Optional rate limiter for auth brute-force protection. */
  rateLimiter?: AuthRateLimiter;
  /** When set, only handle upgrades whose URL path is under this prefix (TopicHub embed). */
  pathPrefix?: string;
}): () => void {
  const {
    httpServer,
  ...
  const addUpgradeListener = pathPrefix
    ? httpServer.prependListener.bind(httpServer)
    : httpServer.on.bind(httpServer);
  const upgradeListener = (req: IncomingMessage, socket: Duplex, head: Buffer) => {
    void (async () => {
      if (pathPrefix) {
        const raw = req.url ?? "/";
