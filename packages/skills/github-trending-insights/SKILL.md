...
- **Install / onboarding path**: commands or steps; prerequisites.
- **Proof assets**: demos, GIFs, benchmarks, comparisons — what lowers skepticism fastest.

#### 5. Positioning & differentiation
- **Category label** (where people “file” it mentally).
- **Closest alternatives** and **sharp differentiators** (1–3 bullets, falsifiable).

#### 6. Heat hypothesis (why it is trending *now*)
- **Wave alignment**: platform shift, regulation, cost pressure, model/API change, etc.
- **Novelty vs utility**: classify and justify with observed signals (issues about production use vs hype threads).

#### 7. Risks & unknowns
- **Maturity**, **license/fork risk**, **vendor/orphan risk**, **security posture** — separate facts from speculation.
```

After all dossiers, add a short **Cross-repo comparison** table (repos as rows; columns like *problem*, *ICP*, *TTV*, *moat type*, *heat driver*) when comparing 3+ repos.

## Caveats

- **Stars ≠ quality**; spikes can be meme, controversy, or astroturfing—note uncertainty when evidence is thin.
- **Megacorp entries can dominate a row** without representing “indie trending culture”; keep them peripheral unless the user asks—this skill is tuned for **organic board signal**.
- **Regional and demographic bias** in who stars and shares.
- Do not treat a single week’s board as a permanent market map; compare **multi-week** or **topic** views when possible.
